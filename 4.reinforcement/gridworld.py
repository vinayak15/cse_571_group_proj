...
if __name__ == '__main__':
    ...
    if opts.agent == 'value':
        ...
        qLearnOpts = {'gamma': opts.discount,
                      ...
                      'epsilon': opts.epsilon,
                      'actionFn': actionFn}
        a = qlearningAgents.SarsaNStepAgent(**qLearnOpts)
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent in ('random', 'value', 'asynchvalue', 'priosweepvalue'):
                def displayCallback(state): return display.displayValues(a, state, "CURRENT VALUES")
            if opts.agent == 'q':
                def displayCallback(state): return display.displayQValues(a, state, "CURRENT Q-VALUES")
            if opts.agent == 's':
                def displayCallback(state): return display.displayQValues(a, state, "CURRENT Q-VALUES")
            if opts.agent == 'slamda':
                def displayCallback(state): return display.displayQValues(a, state, "CURRENT Q-VALUES")
            if opts.agent == 'nstep':
                def displayCallback(state): return display.displayQValues(a, state, "CURRENT Q-VALUES")
