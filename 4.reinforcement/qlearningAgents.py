...
class SarsaNStepAgent(TrueOnlineSarsaLamda):

    def __init__(self, **args):
        "You can initialize Q-values here..."
        TrueOnlineSarsaLamda.__init__(self, **args)
    ...
    def update(self, state, action, nextState, reward):
        ...
        if tau >= 0:
            ...
            temp = self.rewards[tau+1:min(tau+self.n+1, self.T+1)]
            G = 0
            ...
            for i in range(len(temp)):
                ...
                G += (self.discount**(power-tau-1)) * temp[i]

            if tau + self.n < self.T:
                G += self.discount * self.getQValue(nextState, nextAction)

            diff = G - self.getQValue(state, action)
    ...
    def startEpisode(self, state):
        TrueOnlineSarsaLamda.startEpisode(self, state)
        self.visited.clear()
        self.actions = []
        self.rewards = []
        self.states = []
